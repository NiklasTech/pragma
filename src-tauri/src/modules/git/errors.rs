    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            GitError::NotInstalled => {
                write!(f, "git is not available on PATH. Install Git and retry.")
            }
            GitError::TooOld { found, required } => {
                write!(f, "git {found} is too old; needs git {required} or newer.")
            }
            GitError::NotADirectory(p) => write!(f, "not a directory: {p}"),
            GitError::PathOutsideWorkspace(p) => {
                write!(f, "path is outside the authorized workspace: {}", p.display())
            }
            ...
            GitError::NoUpstream => write!(
                f,
                "no upstream configured. Run `git push -u <remote> <branch>` first."
            ),
            GitError::AuthRequired(detail) => {
                write!(f, "authentication required: {detail}. Check your SSH key or HTTPS credentials.")
            }
