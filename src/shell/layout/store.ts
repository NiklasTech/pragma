...
const layoutStoreCreator: StateCreator<FullLayoutTreeState> = crossWindowSync<FullLayoutTreeState>(
  ...
  setTerminalMode: (mode) =>
    set((s) => {
      ...
      // When the terminal is intentionally hidden (not moved to an external
      // window), clean up the underlying PTY sessions.
      if (
        mode === "hidden" &&
        !s.floating.some((f) => findPanelByKind(f.child, "terminal") && f.external)
      ) {
        void useTerminalStore.getState().killAllSessions();
      }
  ...
  moveFloatingToExternal: (floatingId, label) =>
    set((s) => {
      const next = s.floating.map((f) => (f.id === floatingId ? { ...f, external: label } : f));
      return { floating: next, ...markCustomized(s) };
    }),
  dockExternalWindow: (label, bounds) =>
    set((s) => {
      const floating = s.floating.find((f) => f.external === label);
      if (!floating) return s;
      const next = s.floating.map((f) =>
        f.id === floating.id
          ? { ...f, external: undefined, ...(bounds ? { ...f, ...bounds } : {}) }
          : f,
      );
      return { floating: next, ...markCustomized(s) };
    }),
