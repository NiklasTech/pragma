...
function UnifiedDiffView({ patchText }: { patchText: string }) {
  ...
  const getLineClasses = useCallback((type: DiffLineType) => {
    switch (type) {
      case "added":
        return "bg-emerald-500/5 text-emerald-300/90";
      case "removed":
        return "bg-rose-500/5 text-rose-300/90";
      case "hunk":
        return "bg-sky-500/5 text-sky-300/70";
      case "header":
  ...
  const getGutterClasses = useCallback((type: DiffLineType) => {
    switch (type) {
      ...
      case "removed":
        return "bg-rose-500/15 text-rose-400/80";
      case "hunk":
  ...
  return (
    <div ref={scrollRef} className="h-full w-full overflow-auto font-mono text-[13px]">
        ...
        {virtualizer.getVirtualItems().map((virtualItem) => {
          ...
          return (
            ...
            >
              <span
                className={cn(
                  "shrink-0 w-6 text-center text-[11px] select-none leading-5",
