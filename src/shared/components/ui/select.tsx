  ...
  return (
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={<CaretDown className="pointer-events-none size-4 text-fg-subtle" />}
      />
    </SelectPrimitive.Trigger>
  );
}
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn("px-2.5 py-1.5 text-ui-xs font-semibold text-fg-subtle", className)}
      {...props}
    />
  );
}
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-md py-1.5 pr-8 pl-2.5 text-ui-base text-fg-default outline-hidden select-none focus:bg-bg-active focus:text-fg-default not-data-[variant=destructive]:focus:**:text-fg-default data-disabled:pointer-events-none data-disabled:opacity-40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className,
      )}
      {...props}
    >
